...
export default function Register({ onLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const { data } = await api.post('/auth/register', { full_name: form.full_name, email: form.email, password: form.password });
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center px-4">
      <div className="glass rounded-3xl p-8 w-full max-w-md">
        ...
        <p className="text-slate-400 mb-8">Start building your professional portfolio</p>
        ...
        <p className="text-center text-slate-400 text-sm mt-6">Already have an account? <Link to="/login" className="text-blue-400 hover:underline">Sign in</Link></p>
      </div>
    </div>
  );
}
